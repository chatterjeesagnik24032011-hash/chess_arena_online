...
function Home({session,profile,onOpenOnline,onPlayBot}){
 ...
 useEffect(()=>{load()},[])
 const create=async()=>{const white=side==='random'?(Math.random()>.5?session.user.id:null):side==='white'?session.user.id:null;const black=side==='black'?session.user.id:null;const r=await supabase.from('games').insert({white_id:white,black_id:black,status:'waiting',fen:new Chess().fen(),moves:[],white_time:parseTC(onlineTime).minutes*60000,black_time:parseTC(onlineTime).minutes*60000,increment:parseTC(onlineTime).increment}).select().single();if(r.error)setMsg(r.error.message);else onOpenOnline(r.data.id)}
