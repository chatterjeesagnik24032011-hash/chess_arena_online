...
function initialClock(){return {w:10*60*1000,b:10*60*1000}}
...
function App(){
 const [session,setSession]=useState(null),[profile,setProfile]=useState(null),[screen,setScreen]=useState('home'),[mode,setMode]=useState(null),[gameId,setGameId]=useState(null)
 ...
 if(screen==='bot')return <BotGame level={mode||'beginner'} profile={profile} onBack={()=>{setScreen('home');setMode(null)}}/>
 return <Home session={session} profile={profile} onOpenOnline={id=>{setGameId(id);setScreen('online')}} onPlayBot={l=>{setMode(l);setScreen('bot')}}/>
}

function Home({session,profile,onOpenOnline,onPlayBot}){
 const [tab,setTab]=useState('play'),[code,setCode]=useState(''),[side,setSide]=useState('random'),[msg,setMsg]=useState(''),[games,setGames]=useState([])
 ...
 useEffect(()=>{load()},[])
 const create=async()=>{const white=side==='random'?(Math.random()>.5?session.user.id:null):side==='white'?session.user.id:null;const black=side==='black'?session.user.id:null;const r=await supabase.from('games').insert({white_id:white,black_id:black,status:'waiting',fen:new Chess().fen(),moves:[],white_time:600000,black_time:600000,increment:0}).select().single();if(r.error)setMsg(r.error.message);else onOpenOnline(r.data.id)}
 const join=async()=>{const id=code.trim();if(!id)return;const r=await supabase.from('games').select('*').eq('id',id).single();if(r.error){setMsg('Game not found.');return}if(r.data.white_id&&r.data.black_id){setMsg('That game already has two players.');return}const patch=r.data.white_id?{black_id:session.user.id,status:'active'}:{white_id:session.user.id,status:'active'};const u=await supabase.from('games').update(patch).eq('id',id);if(u.error)setMsg(u.error.message);else onOpenOnline(id)}
 return <div className="app"><header className="topbar"><div className="brand">♞ CHESS ARENA</div><nav><button className={tab==='play'?'active':''} onClick={()=>setTab('play')}>Play</button><button className={tab==='history'?'active':''} onClick={()=>setTab('history')}>History</button></nav><div className="user">{profile?.display_name||session.user.email}<button className="ghost" onClick={()=>supabase.auth.signOut()}>Log out</button></div></header>{tab==='play'?<main className="dashboard"><section className="hero"><div><div className="eyebrow">ONLINE CHESS ARENA</div><h1>Play chess your way.</h1><p>Challenge another player online or train against three bot levels.</p></div><div className="hero-badge">10+0<br/><span>standard</span></div></section><section className="mode-grid"><div className="mode-card online-card"><div className="card-icon">♟</div><h2>Play Online</h2><p>Create a room and share the game ID with your opponent.</p><div className="row"><select value={side} onChange={e=>setSide(e.target.value)}><option value="random">Random side</option><option value="white">Play White</option><option value="black">Play Black</option></select><button className="primary" onClick={create}>Create game</button></div><div className="join"><input value={code} onChange={e=>setCode(e.target.value)} placeholder="Game ID"/><button className="secondary" onClick={join}>Join</button></div></div><div className="mode-card"><div className="card-icon">♛</div><h2>Play vs Bot</h2><p>Use the same board, clock and review tools as your Python version.</p><div className="bot-grid">{Object.entries(BOT).map(([k,b])=><button key={k} onClick={()=>onPlayBot(k)}><b>{b.name}</b><span>{k==='beginner'?'Easy':k==='intermediate'?'Medium':'Strong'}</span></button>)}</div></div></section>{msg&&<div className="notice">{msg}</div>}</main>:<main className="dashboard"><section className="panel"><h2>Game history</h2>{games.length?games.map(g=><button className="history-row" key={g.id} onClick={()=>onOpenOnline(g.id)}><span>{g.id.slice(0,8)}…</span><span>{g.status}</span><span>{new Date(g.updated_at).toLocaleString()}</span></button>):<p className="muted">No online games yet.</p>}</section></main>}</div>
}
...
function BotGame({level,profile,onBack}){
 const [chess,setChess]=useState(()=>new Chess()),[selected,setSelected]=useState(null),[targets,setTargets]=useState([]),[items,setItems]=useState([]),[current,setCurrent]=useState(-1),[clock,setClock]=useState(initialClock()),[orientation,setOrientation]=useState('w'),[status,setStatus]=useState('active'),[thinking,setThinking]=useState(false),[lastMove,setLastMove]=useState(null),[engineCp,setEngineCp]=useState(0),[stockfishReady,setStockfishReady]=useState(false)
 const botColor='b'
 useEffect(()=>{let cancelled=false;analyzeFen(chess.fen(),{depth:10}).then(r=>{if(!cancelled){setEngineCp(r.cp);setStockfishReady(true)}}).catch(()=>{});return()=>{cancelled=true}},[chess])
 useEffect(()=>{
  if(status!=='active'||chess.turn()!==botColor||thinking)return
  let cancelled=false
  setThinking(true)
  const depth=level==='beginner'?5:level==='intermediate'?9:13
  const delay=level==='beginner'?250:level==='intermediate'?450:700
  const timer=setTimeout(async()=>{
    try{
      const result=await analyzeFen(chess.fen(),{depth})
      if(!cancelled && result?.bestmove && result.bestmove!=='(none)') commit(result.bestmove,true)
    }catch(e){
      if(!cancelled) console.error('Bot engine error',e)
    }finally{
 ...
 function commit(uci,isBot=false){const c=new Chess(chess.fen());let mv;try{mv=c.move({from:uci.slice(0,2),to:uci.slice(2,4),promotion:uci.slice(4)||'q'})}catch{return}const before=chess.fen();const analysis=isBot?{label:'BEST MOVE',loss:0,rank:1,bestMove:uci}:{...classifyMove(before,uci,mv.san,1)};const item={ply:items.length+1,san:mv.san,uci,label:analysis.label,loss:analysis.loss,rank:analysis.rank,bestMove:analysis.bestMove,at:Date.now(),by:isBot?'bot':'human'};setItems(x=>[...x,item]);setCurrent(items.length);setLastMove({from:uci.slice(0,2),to:uci.slice(2,4)});setChess(c);setSelected(null);setTargets([]);if(c.isGameOver())setStatus(c.isCheckmate()?(c.turn()==='w'?'black-won':'white-won'):'draw')}
 ...
 return <GameFrame title={`VS ${BOT[level].name.toUpperCase()}`} profile={profile} onBack={onBack} clocks={<Clock clock={clock} turn={chess.turn()} />} board={<Board chess={chess} orientation={orientation} selected={selected} legalTargets={targets} lastMove={lastMove} engineCp={engineCp} onSquare={square}/>} leftTools={<><button onClick={()=>setOrientation(o=>o==='w'?'b':'w')}>Flip board</button><button onClick={()=>{setChess(new Chess());setItems([]);setClock(initialClock());setStatus('active');setLastMove(null);setThinking(false)}}>New game</button></>} right={<><div className="engine-card"><span>ENGINE</span><b>{thinking?'Thinking…':scoreLabel(engineCp)}</b><small>{stockfishReady?'Stockfish 19 Lite':'Arena fallback'} · bot depth {BOT[level].depth}</small></div><ReviewPanel items={items} current={current} onJump={i=>setCurrent(i)}/></>} status={status}/>
}
